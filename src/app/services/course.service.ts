...
import { Course, Language, Unit, Song } from '../models/course'
...
export class CourseService {
    ...
    update(course: Course): Promise<Course> {
        ...
        return this.http
            .post(url, JSON.stringify({ name: course.name, description: course.description, language_id: +course.language_id }), this.options)
            .toPromise()
            .then(() => course)
            .catch(this.handleError);
    }
    ...
    createUnit(unit: Unit): Promise<Unit> {
        ...
        return this.http.post(url,
            JSON.stringify({ name: unit.name, description: unit.description, course_id: unit.course_id, weight: unit.weight }),
            this.options)
            .toPromise()
    ...
    createSong(song: Song): Promise<Song> {
        ...
        return this.http.post(url,
            JSON.stringify({
                title: song.title, album: song.album, artist: song.artist, description: song.description, lyrics: song.lyrics, file_name: song.file_name,
                file_type: song.file_type, embed: song.embed, weight: song.weight, unit_id: song.unit_id
            }),
            this.options)
            .toPromise()
